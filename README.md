...
### 🔗 连线系统
- **6 种语义连线类型**：依赖、矛盾、转化、启发、否定、使用方法(这部分并未实现)
- **灵活锚点**：节点锚点（带 4 边自动对齐）和自由画布锚点(自由锚点未实现)
...
### ⚡ 操作与编辑
- **Undo/Redo**：大部分节点/连线操作均支持撤销重做（命令模式实现）
  - 新增/删除/移动/缩放节点
  - 编辑节点文字
  - 修改节点颜色/连线颜色
  - 修改连线文字
...
- **两种存档方式**：
  - `.warroom` 单文件：所有内容在一个 JSON 文件中(目前请不要尝试)
...
- 项目目前很多功能还未实现，所以实际上很简陋，不过常用功能基本上都实现了
- 项目大部分源代码由AI编写，我只是进行设计、代码审查和相关测试罢了
