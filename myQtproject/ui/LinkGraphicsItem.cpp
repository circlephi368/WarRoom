...
LinkGraphicsItem::LinkGraphicsItem(const warroom::Uuid& linkId,
	...
	, m_linkId(linkId)
	, m_model(model)
	, m_globalLinkColor(WarRoomMainWindow::getLinkColor())
{
...
{
	// [DESTDBG] 记录 linkId 与 this 地址，便于跟踪连线项的销毁顺序
	qDebug().nospace().noquote()
		<< "[DESTDBG] >>> ~LinkGraphicsItem ENTER this=" << static_cast<void*>(this)
		<< " linkId=" << QString::fromStdString(m_linkId)
		<< " | m_model=" << static_cast<void*>(m_model);

	// 先断开所有信号连接，防止外部回调到正在析构的对象
	disconnect();
	m_model = nullptr;

	qDebug().nospace().noquote()
		<< "[DESTDBG] <<< ~LinkGraphicsItem EXIT this=" << static_cast<void*>(this);
}
	...
	QWidget*)
{
	...
	QColor baseColor = link->color.empty()
		? m_globalLinkColor   // 没有自定义颜色时使用全局默认连线颜色
...
void LinkGraphicsItem::buildPath(QPainterPath& path) const
{
	if (m_wayPoints.isEmpty()) {
		// 查询出入 edge
		const warroom::WarLink* link = m_model ? m_model->getLink(m_linkId) : nullptr;
		int fromEdge = -1;
		int toEdge = -1;
		if (link) {
...
// 刷新全局连线默认颜色
// ---------------------------------------------------------------------------
void LinkGraphicsItem::refreshLinkColor(const QColor& color)
{
	m_globalLinkColor = color;
	update();
}
