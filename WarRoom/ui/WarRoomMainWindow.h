...
// 当前文档统计信息（用于"当前文档统计数据"对话框）
struct DocumentStats {
	int     nodeCount = 0;      // 总节点数（不含文档根）
	int     charCount = 0;      // 总字数（full_text 字符数，含空格/Markdown 符号）
	qint64  fileSize  = 0;      // 存档体积（字节，未保存=0）
	QString fileName;           // 存档文件名（未命名时"未命名"）
	QString filePath;           // 完整路径（未命名时空串）
};
...
private:
	...
	// ---- 文档统计 ----
	// 递归统计子树累计节点数/字数
	void accumulateStats(const std::string& nodeId, int& outNodeCount, int& outCharCount) const;
	// 组装当前文档完整统计信息（含文件体积/文件名）
	DocumentStats computeDocumentStats() const;
