...
// ==================== 构造函数 ====================
NodeGraphicsItem::NodeGraphicsItem(const std::string& nodeId, const warroom::WarNode& node, const std::string& title,
	const std::string& fullText, const QColor& color,
	warroom::NodeKind kind, bool isCollapsed, QGraphicsItem* parent)
	: QGraphicsObject(parent), m_nodeId(nodeId),m_node(node), m_title(title),
...
{
	return QRectF(0, 0, getNodewidth(), getNodeheight());
}
